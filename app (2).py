...
def call_ai(prompt, system_prompt="", max_tokens=1500):
    if not st.session_state.gemini_configured:
        return "⚠️ Chưa có API key. Nhập Groq API key ở sidebar (lấy free tại console.groq.com)."
    try:
...
with st.sidebar:
    ...
    api_input = st.text_input("", type="password", placeholder="gsk_...", label_visibility="collapsed", value=st.session_state.api_key)
    if st.button("Lưu & kết nối", use_container_width=True):
        if configure_gemini(api_input):
            st.success("✅ Kết nối thành công!")
        else:
            st.error("Key không đúng!")
    st.caption("Lấy free tại console.groq.com")
